...
// this.#json1Div.on('click', '.doctor-data', (event) => {
//     const selectedDoctorId = $(event.target).data('id');
//     const selectedDoctorData = this.#doctorsData.find((doctor) => doctor.id === selectedDoctorId);
  
//     this.#json1Div.empty();
//     this.#json1Div.append(`<div>Doctor: ${selectedDoctorData.name}</div>`);
//   });
  
  // ...
  
  // Append doctor data to #json1Div
//   this.#jsonDataCategory.forEach((category) => {
//     if (category.name === 'Doctors') {
//       this.#doctorsData.forEach((doctor) => {
//         this.#json1Div.append(`<div class="doctor-data" data-id="${doctor.id}">${doctor.name}</div>`);
//       });
//     }
//   });
